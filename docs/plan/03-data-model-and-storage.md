...
- **Single writer by design**: no busy-timeout choreography, no cross-process locking, no killed-mid-write recovery scenarios. The appex is a stateless IPC client with zero database code.
...
- **No runtime secrets**: the Developer Preview has no model client or API-key storage. Nothing learning-related lives in UserDefaults.

### R2 spike (must run in Phase 0, see [08-roadmap.md](08-roadmap.md))

The old R2 (cross-process SQLite) is eliminated by D9. The Phase 0 spike tested sandboxed app-extension → app request/reply, latency, and app-down behavior. It also disproved the original NSXPC transport assumption; the verified outcome below is now the shipped design. The Store API remains behind a protocol so a future transport change would not touch storage callers.
...
CREATE TABLE enrichment (               -- historical/reserved; unused by Developer Preview
  itemId       TEXT NOT NULL,
  kind         TEXT NOT NULL,
...
CREATE TABLE settings (                 -- sync-relevant flags only; no secrets
  key   TEXT PRIMARY KEY,               -- activeLanguage, enabled,
...
- Item IDs are **content-addressed slugs** (`de.word.haus`, `de.chunk.es-gibt`) minted by packtool and stable across pack versions. Progress joins on `itemId`, so importing a newer pack preserves all progress.
...
- Ring buffer per item: keep the newest **5** sentences per item, cap total store at ~2,000 rows. Captured sentences stay device-local.
