...
│  │ native message ⇄ CFMessagePort; NO database access;   │         │
│  │ launches the app only for explicit openDashboard      │         │
│  └────────────────────────────▲─────────────────────────┘         │
...
**The app is the server.** It runs as a menu bar app registered as a login item (`SMAppService`), owns the SQLite database exclusively, and vends a JSON-envelope API over an App-Group-prefixed `CFMessagePort`. The appex holds no state and never touches the database — it translates native messages into CFMessagePort requests and back. Data flows one way per concern: **vocab flows down** (DB → snapshot → IPC → background cache → content script), **events flow up** (content script → background queue → native message → IPC → DB), and **UI reads the DB in-process** (GRDB `ValueObservation` gives the SwiftUI dashboard live queries — no cross-process signaling needed).
...
**The app** is a SwiftUI shell over LearnerCore: Overview (real data only, P4), Practice (the one review engine UI), Library (pack browser + item detail), and Settings (per-site rules, pack import, and practice controls). It has no runtime LLM or network path.

**The appex handler** is a stateless forwarder: decode envelope → forward over CFMessagePort to the app → encode response. It links only the `Sync` message types from LearnerCore, holds no state between invocations (Safari may cold-start and kill it freely), and never opens the database.
...
- **App**: long-lived — a menu bar app registered as a login item, so it is effectively always running. Sole owner of the GRDB `DatabasePool`; the SwiftUI dashboard observes queries live via `ValueObservation` (in-process, which is now the only process that matters). Registers its App-Group-prefixed CFMessagePort at launch. Menu bar affordances: practice actions, status, pause toggle, navigation, and quit.
- **Appex**: cold-started per message batch by Safari, possibly killed immediately after. Therefore: decode → CFMessagePort call (connecting per invocation) → encode → return. If the app is unavailable, background sync returns `appUnavailable`; only explicit `openDashboard` intent may launch the app and retry. No caches, no database, no background work.
...
Everything under `extension/src/core/` is WebExtension-standard and browser-agnostic. The only Safari-specific code is `src/adapters/safari/` (native messaging + storage cache). The app-as-server design makes a future Chrome port cleaner: another adapter could talk to **the same envelope API** over a separately authenticated transport. Localhost HTTP is deliberately not used for Safari; the current App-Group-prefixed CFMessagePort lookup is constrained by the shared sandbox entitlement. **Rule: no `sendNativeMessage` call outside the adapter directory** — enforced by a lint rule in the extension build.
...
| D6 | **Model access belongs to a future authoring CLI, not the runtime** | Keeps the shipped app local-only and makes generated pack source reviewable | Tutor/provider configuration in the app |
| D7 | **Authoring secrets are contributor-local** | Never enter pack source, build output, the app DB, or repository | committed keys / runtime settings |
...
| D9 | **App-as-server**: menu bar app owns the DB exclusively; appex is a stateless CFMessagePort client using an App-Group-prefixed port | Eliminates cross-process SQLite entirely; single writer; live in-process observation; appex trivially thin; the envelope API can serve another transport later. Cost: app-down state — mitigated by login item, explicit launch for `openDashboard`, cached snapshot + queued events | Shared cross-process SQLite from the appex; `NSXPCListener(machServiceName:)`, which did not register reliably outside Xcode; localhost HTTP for Safari |
...
- Future agent/LLM pack authoring boundary: [06-llm-integration.md](06-llm-integration.md)
